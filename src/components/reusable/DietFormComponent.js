          ...
          <form role="form" onSubmit={(e) => e.preventDefault()}>
            <fieldset>
              <div className="form-group">
                <InputField styleClass="form-control" inputType="text" name="descricao" value={props.values.descricao} maxLength="75" fieldName="Descrição" onChange={event => props.hasChanged(event)} />
              </div>
              <div className="form-group">
                <InputField styleClass="form-control" inputType="number" min={1} name="frequencia_diaria" value={props.values.frequencia_diaria} fieldName="Frequência Diária" onChange={event => props.hasChanged(event)} />
              </div>
              <div className="form-group">
                <InputField styleClass="form-control" inputType="number" min={100} name="qtde_racao" value={props.values.qtde_racao} fieldName="Quantidade por Porção (g)" onChange={event => props.hasChanged(event)} />
              </div>
              <div className="form-group">
                <InputField styleClass="form-control" inputType="date" name="data_inicio" value={props.values.data_inicio} fieldName="Data Início" onChange={event => props.hasChanged(event)} />
              </div>
              <div className="form-group">
                <InputField styleClass="form-control" inputType="date" name="data_fim" value={props.values.data_fim} fieldName="Data Fim" onChange={event => props.hasChanged(event)} />
              </div>
              <div className="form-group">
              ...
              <span style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-around' }}>
                <button className="btn btn-warning" style={{width: '100%', height: '100%'}} onClick={event => props.handleSubmit("dietas")} type="submit" name="submit-btn">Alterar</button>              
              </span>
            </fieldset>
          </form>
        </div>
